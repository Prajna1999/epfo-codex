...
export function PortalTopbar({
  ...
  profile = {
    ...
    accountId: "UAN 1009 2000 0123",
    accountStatus: "KYC verified",
  },
}: {
  mobileOpen: boolean;
  onToggleMobile: () => void;
  ...
  onOpenAgent?: () => void;
  agentLabel?: string;
  agentAlert?: { title: string; detail: string; onOpen: () => void };
  onLogout?: () => void;
  profile?: {
    initials: string;
    name: string;
  ...
  const voiceNumber = process.env.NEXT_PUBLIC_TWILIO_IVR_NUMBER || "14470";
  return (
    <header className="topbar">
      <div className="brand">
        ...
        <div>
          <div className="brand-title">EPFO</div>
          <div className="brand-sub">{t("Your retirement account")}</div>
        </div>
      </div>
      <div className="topbar-actions">
        ...
        <div className="notification-wrap">
          <button
            ...
            aria-controls="contribution-notification"
          ...
          {notificationsOpen && (
            ...
            >
              {agentAlert && (
                <button
                  type="button"
                  className="notification-row is-alert"
                  onClick={() => {
                    agentAlert.onOpen();
                    setNotificationsOpen(false);
                  }}
                >
                  <span className="contribution-icon alert">
                    <Icon name="briefcase" size={20} />
                  </span>
                  <div>
                    <small>{t("ACCOUNT MOVE")}</small>
                    <strong>{agentAlert.title}</strong>
                    <span>{agentAlert.detail}</span>
                  </div>
              ...
              <div className="notification-row">
                ...
                <div>
                  <small>{t("LATEST CONTRIBUTION")}</small>
                  <strong>{t("August contribution received")}</strong>
                  <span>Infosys Limited · {t("Deposited 18 August")}</span>
                </div>
                <b>+ ₹8,430</b>
              </div>
            </section>
        ...
        <div className="account-menu">
          <button
            ...
            aria-label={
              onOpenAccount ? t("View profile details") : profile.name
            }
          >
          ...
          <section className="account-hover" aria-label="Account summary">
            <strong>{profile.accountId ?? "UAN 1009 2000 0123"}</strong>
            <span>{profile.accountStatus ?? "KYC verified"}</span>
            {onLogout && (
      ...
      <details className="sidebar-help" style={{ display: "block" }}>
        <summary>
          ...
          <span>
            ...
            <small>{t("Guides and support")}</small>
          </span>
          <Icon name="chevron" size={16} />
        </summary>
        ...
        <ul>
          <li>Keep your UAN and registered mobile ready.</li>
          <li>Track requests from Claims or Profile.</li>
          <li>Never share an OTP over a call.</li>
        </ul>
      </details>
    </aside>
  );
